...
import { Search, ChevronRight, Edit2, Trash2, FileSignature } from 'lucide-react';
import { AddNewButton } from '../../components/ActionButtons';
import { Link, useNavigate } from 'react-router-dom';
import Swal from 'sweetalert2';
import toast from 'react-hot-toast';

export default function DraftBlogsView() {
  ...
  // Fetch blogs (Drafts only) and categories
  const fetchData = async () => {
    try {
      const [blogRes, catRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL}/api/blogs?status=Draft`),
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res.ok) {
            Swal.fire('Deleted!', 'Draft blog has been deleted.', 'success');
  ...
  return (
    <div className="p-6 font-sans bg-[#f3f6f9] min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <h4 className="text-[15px] font-medium text-gray-700 uppercase tracking-wide">Drafts Grid View</h4>
        <div className="text-[13px] text-gray-500">
          <span>Blogs</span> <span className="mx-1">/</span> <span className="text-gray-400">Drafts</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          ...
          {filteredBlogs.length === 0 ? (
            <div className="bg-white rounded-[0.25rem] border border-dashed border-[#ced4da] shadow-sm p-12 flex flex-col items-center justify-center text-gray-500">
              <FileSignature size={48} className="text-gray-300 mb-4" />
              <h3 className="text-lg font-medium text-gray-700 mb-1">No Drafts Found</h3>
              <p className="text-sm">You don't have any blog posts saved as drafts.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentBlogs.map((blog) => (
                <div key={blog._id} className="group bg-white rounded-[0.25rem] border-2 border-dashed border-[#ced4da] overflow-hidden transition-all hover:border-[#405189] hover:shadow-md relative flex flex-col h-full">
                  
                  {/* Hover Action Icons */}
                  <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                    <button 
                      onClick={() => navigate('/blogs/add', { state: { blogData: blog } })}
                      className="p-1.5 bg-blue-50 text-blue-500 rounded hover:bg-blue-500 hover:text-white transition-colors border border-blue-100 shadow-sm"
                      title="Edit Draft"
                    >
                      <Edit2 size={16} />
                    </button>
                    <button 
                      ...
                      className="p-1.5 bg-red-50 text-red-500 rounded hover:bg-red-500 hover:text-white transition-colors border border-red-100 shadow-sm"
                      title="Delete Draft"
                    >
                      <Trash2 size={16} />
                    </button>
                  ...
                  <div className="w-full h-40 relative flex-shrink-0">
                    <img src={blog.image || 'https://placehold.co/400x250/e0e0e0/555?text=No+Image'} alt={blog.title} className="w-full h-full object-cover p-2 rounded-t-lg" />
                  </div>
                  
                  <div className="p-4 flex flex-col flex-grow">
                    <h4 className="text-[15px] font-semibold text-[#495057] mb-2 hover:text-[#405189] cursor-pointer transition-colors line-clamp-2">
                      {blog.title}
                    </h4>
                    
                    <div className="text-[13px] text-gray-500 mb-4 line-clamp-3 flex-grow" dangerouslySetInnerHTML={{__html: blog.desc}}></div>
                    
                    <div className="flex justify-between items-center mt-auto pt-4 border-t border-dashed border-gray-200">
                       <Link to={`/blogs/view/${blog._id}`} className="text-[13px] font-medium text-[#405189] hover:underline inline-flex items-center">
                        Preview <ChevronRight size={14} className="ml-1" />
                      </Link>
                      <span className="text-[10px] uppercase font-bold tracking-wider text-gray-400 bg-gray-100 px-2 py-1 rounded">
                        {blog.category}
                      </span>
                    </div>
                  </div>
